...
export async function sendTelegramMessage(token: string, chatId: string, message: string, parseMode: "Markdown" | undefined = "Markdown") {
  ...
  return runLoggedApi("Telegram SendMessage", async () => {
    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), TIMEOUT)
    try {
      const body: Record<string, unknown> = { chat_id: chatId, text: message }
      if (parseMode) body.parse_mode = parseMode
      const response = await fetch("https:" + "//api.telegram.org/bot" + token + "/sendMessage", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body), signal: controller.signal })
...
export function sendWeeklyReportFailureNotification(token: string, chatId: string, startDate: string, endDate: string, totalActivities: number, totalLeadershipEvents: number, errorMessage: string) {
  return sendTelegramMessage(token, chatId, ["❌ Tạo báo cáo Sales Weekly thất bại", `📅 Kỳ báo cáo: ${startDate} — ${endDate}`, `📊 Activities đã đọc: ${totalActivities}`, `🗓️ Lịch lãnh đạo đã đọc: ${totalLeadershipEvents}`, `⚠️ ${describeReportError(errorMessage)}`, "🛑 Không có báo cáo hoặc thông báo thành công nào được tạo."].join("\n"), undefined)
}
