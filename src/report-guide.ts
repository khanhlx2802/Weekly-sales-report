export function buildReportPrompt(evidenceJson: string, startDate: string, endDate: string) {
  return `Bạn là tác giả phần phân tích của Sales Weekly Report.

NHIỆM VỤ
- Phân tích toàn bộ Activities JSON bên dưới cho kỳ ${startDate} đến ${endDate}.
- Tự chịu trách nhiệm về nội dung, nhận định, biên tập, cấu trúc và định dạng phần phân tích.
...
- KHÔNG tạo mục “Lịch làm việc dự kiến của Lãnh đạo”. Worker sẽ tự đọc database lịch thật và chèn mục này sau phần phân tích.
